...
1. **Check for existing system:**
   ...
   - If it exists: "Design system already exists. Use `/chisel carve` to tweak it or `/chisel exhibit` to regenerate the showcase."
...
4. **If Stitch MCP is available, offer it:**
   ...
   If user says yes:
   - Use Stitch to generate 2-3 screen variants
...
/* Regenerate with: /chisel exhibit */
...
4. **When user approves ("looks good", "let's go", "ship it"):**
   ...
   - Remind: "Use `/chisel carve` to tweak later, `/chisel inspect` to audit code against the system."
